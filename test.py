...
column_names =      ['Fran Datum Tid (UTC)', 'till', 'day', 'temperature', 'Kvalitet', 'Tidsutsnitt:', 'Unnamed: 5']
column_names_used = ['Fran Datum Tid (UTC)', 'till', 'day']

def make_numeric_values(arr, title):            # make numeric values from ___
    new_arr = []                                # new array
    for date in arr[title]:                     # 
        new_date = make_date(date)              # make new date
        new_arr.append(new_date)                # add new date to new array
    arr[title] = new_arr
...
def make_date(date):
    new_date = date.split(' ')                  # everytime as ' ' appers -> split
    new_date = new_date[0]
    new_date = new_date.split('-')              # everytime as '-' appers -> split
    new_number = ''
    first = True
...
def train():
    ...
    data1 = pd.read_csv(dataset_url1, sep=';', skiprows=3607, names=column_names)   # with pandas read a comma-separated values (csv) file into DataFrame
    data2 = pd.read_csv(dataset_url2, sep=';', skiprows=15, names=column_names)     # file path (URL or local), 
...
def run_menu():
    print("*" *48)                                                      # 48 times '*'
    print("-" *10 + " What would you like to do? " + "-" * 10)          # 10 times '-' ... '-'
    print("\n")
    print("1. Look up the weather on a specific day")
    print("2. Predict the weather on a specific day")
    print("3. Train data for weather prediction")
    print("\n")

    option = input("Enter option: ")

    while True:
        if option == 1 or option == 2 or option == 3 or option == 9:    # if this -> break and return value
            break
        option = input("Enter option: ")                                # if anything else - repeat command
    return option
...
if __name__== "__main__":
    ...
    while True:
        option = run_menu()
        if option == 9:                                                 # if 9 -> turn off = break
            break
        if option == 3:                                                 # if 3 -> train model
            train()
        else:
            run_program(option)
